...
\begin{remark}[Uniqueness and the loop gain at $f = 1$]
\label{rem:plateau}
The clamp fixes the loop gain at $g = f \leq 1$
(\cref{sec:existence:mono}), which makes $E$ non-increasing but permits
it to be flat; the two regimes part at the boundary. When $f < 1$
strictly, $E$ loses a full wei of altitude every $1/(1-f)$ steps on
average, so the root is unique --- in real arithmetic, and within a wei
of it on the lattice. When $f = 1$ the return $F(P_A(x/N))$ can track
$x$ exactly, and the zero set $\{x : E(x) = 0\}$ widens into a
contiguous integer block: a \emph{plateau} of self-consistent NAVs in
place of a single root. Strict uniqueness therefore carries the
hypothesis $f < 1$, which the clamp alone does not supply.

This costs nothing, because the published value is \emph{defined} to be
...
$\JTeff = \STraw + P_A(x/N) - x$. A unit step inside the block raises
$F(P_A(x/N))$ by exactly $1$ (the $-x$ term fell by $1$ and $E$ held at
$0$); as $F$ is $1$-Lipschitz and $P_A$ steps by $0$ or $1$
(\cref{sec:existence:mono}), this forces $P_A$ up by exactly $1$, whence
\[
  \JTeff(x{+}1) - \JTeff(x)
...
construction (\cref{sec:overview:waterfall}). Existence does not degrade
at $f = 1$ either: the $+1$ slack in $x_{\max}$
(\cref{sec:existence:sign}) makes $E(x_{\max}) \leq -1 < 0$ whatever the
gain, so the bracket still straddles a crossing. Strict uniqueness needs
...
drains, the published mark stays correct --- it reads the frozen curve,
not live balances --- but tracking that drift is a separate operational
concern. So is Royco's coverage and correlation parameter, a distinct
